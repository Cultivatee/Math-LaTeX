...
\subsubsection*{16题}
\paragraph{题目} 若 $\{f_k(x)\}$ 在 $E$ 上依测度收敛到 $f(x)$，证明 $\{|f_k(x)|\}$ 在 $E$ 上依测度收敛到 $|f(x)|$；又若 $f_k(x)\le g(x)$，$k=1,2,\ldots$，几乎处处于 $E$，证明 $f(x)\le g(x)$ 几乎处处于 $E$。

\paragraph{答案} 由不等式
\[
 \big||f_k(x)|-|f(x)|\big|\le |f_k(x)-f(x)|
\]
可知，对任意 $\eps>0$，
\[
 E\big(\big||f_k|-|f|\big|\ge\eps\big)
\subset E(|f_k-f|\ge\eps).
\]
两边取测度并令 $k\to\infty$，得
\[
 mE\big(\big||f_k|-|f|\big|\ge\eps\big)\to0.
\]
故 $|f_k|\to |f|$ 依测度。

再证第二个结论。由依测度收敛，可取子列 $f_{k_j}$，使得
\[
 f_{k_j}(x)\to f(x)\qquad \text{a.e. on }E.
\]
又对每个 $j$，$f_{k_j}(x)\le g(x)$ 几乎处处成立。去掉这些零测例外集及子列不收敛的零测集后，在剩余点上有
\[
 f_{k_j}(x)\le g(x),\qquad f_{k_j}(x)\to f(x).
\]
令 $j\to\infty$，得 $f(x)\le g(x)$。故 $f\le g$ 几乎处处于 $E$。

\subsubsection*{26题}
\paragraph{题目} 对 $x\in[0,1]$，定义
\[
 f(x)=\max\{a_i:i\in\mathbb{N},\ x=0.a_1a_2\cdots\text{ 为 }x\text{ 的十进制小数表示}\},
\]
其中十进制表示不取 $9$ 作为循环节。证明：$f(x)$ 可测，且几乎处处等于常数。

\paragraph{答案} 令
\[
 f_k(x)=a_k(x),
\]
即 $x$ 的第 $k$ 位十进制数字。每个 $f_k$ 只取 $0,1,\ldots,9$ 这十个值，并且每个取值集合都是有限个区间的并加上有限个端点，故 $f_k$ 是可测的简单函数。又
\[
 f(x)=\sup_{k\ge1}f_k(x),
\]
所以 $f$ 可测。

下面证明 $f=9$ 几乎处处成立。不妨只考虑 $[0,1)$，点 $1$ 不影响测度。令
\[
 F_N=\{x\in[0,1):x\text{ 的前 }N\text{ 位十进制数字都不是 }9\}.
\]
每一位数字不是 $9$ 的选择有 $9$ 种，故
\[
 mF_N=\left(\frac9{10}\right)^N.
\]
令
\[
 F=\bigcap_{N=1}^{\infty}F_N.
\]
则 $F$ 是所有十进制展开中从未出现数字 $9$ 的点集。由测度连续性，
\[
 mF=\lim_{N\to\infty}mF_N=\lim_{N\to\infty}\left(\frac9{10}\right)^N=0.
\]
在 $[0,1)\setminus F$ 上，十进制展开中至少有一位数字为 $9$，因此
\[
 f(x)=9.
\]
所以 $f(x)=9$ 几乎处处成立。
