...
\begin{question}
	...
	（$PA=AC=2$，$\angle PAC=45^\circ$，$\angle B=90^\circ$，$\angle PCB=105^\circ$，折起使面$PAC\perp$面$ABC$；(1)$BC\perp AD$.）
\end{question}
...
\begin{minipage}[t]{0.3\textwidth}
	\begin{tikzpicture}[scale=0.8, baseline=(current bounding box.north)]
		...
		\coordinate (P) at (0,2);
		\coordinate (E) at (-0.9,1.2);
		\coordinate (D) at (-1.1,0.1);
		
		% 坐标轴
		\draw[->, gray!40] (A) -- (0.5,-1.7) node[right] {$y$};
		\draw[->, gray!40] (A) -- (0,2.7) node[above] {$z$};
		\draw[->, gray!40] (A) -- (1.3,0.7) node[right] {$x$};
...
\category{点关于线对称}
\begin{question}
	$A(2,1)$，$l:x-y-4=0$，求$A$关于$l$的对称点$B(a,b)$.
\end{question}
	...
	\begin{center}
		\begin{tikzpicture}[scale=0.7, baseline=(current bounding box.north)]
			...
			\begin{scope}[shift={(-6,0)}]
				...
				\node[below] at (1.6,-1.6) {$\begin{cases} l_1\parallel l_2 \\ d_{A\to l_1}=d_{A\to l_2} \end{cases}$};
			\end{scope}
			% 三平行线等距
			\begin{scope}[shift={(-1.5,0)}]
				...
				\node[below] at (1.7,-1.6) {$\begin{cases} l_1\parallel l_2\parallel l \\ d_{l_1\to l}=d_{l_2\to l} \end{cases}$};
			\end{scope}
			% 角平分线对称
			\begin{scope}[shift={(3.5,0)}]
				\draw[thick] (-1,-1.5) -- (2,2) node[above] {$l_1$};
				\draw[thick] (-1,1) -- (2.5,-1.5) node[right] {$l_2$};
				\draw[thick] (-1.5,0) -- (2.5,0.4) node[right] {$l$};
				\fill (0.3,0) circle (2pt) node[below] {$P$};
				\fill (1.2,0.9) circle (2pt) node[left] {$A$};
				\fill (1.4,-0.7) circle (2pt) node[below] {$B$};
				\draw[dashed] (1.2,0.9) -- (1.4,-0.7);
				\node[below] at (1,-1.6) {$\begin{cases} l_2\text{和}l_1\text{关于}l\text{对称} \\ A\text{关于}l\text{对称点}B \end{cases}$};
			\end{scope}
		\end{tikzpicture}
	\end{center}
\end{knowledge}
...
\begin{knowledge}{设方程办法}
	已知圆心、半径 $\to$ 标准（几何）；已知圆上两点 $\to$ 圆心在中垂线上.
...
\begin{knowledge}{最值结论}
	...
	圆系方程：两圆相减出公共弦方程；一圆方程$+\lambda\cdot$公共弦方程$=0$.
\end{knowledge}
